...
const EducationCard = ({ item, index }) => (
  <div
    ...
    className="relative bg-white/5 backdrop-blur-md border border-white/10 rounded-3xl p-8 md:p-10 hover:border-red-500/30 hover:shadow-[0_20px_50px_rgba(255,42,42,0.12)] transition-all duration-500 group"
  >
    {/* Accent bar */}
    <div className="absolute left-0 top-8 bottom-8 w-1 bg-gradient-to-b from-[#ff2a2a] to-red-800 rounded-full opacity-60 group-hover:opacity-100 transition-opacity" />
    ...
    <div className="pl-5">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-4">
        ...
        <span className="text-white/40 text-xs font-mono font-bold tracking-wider">{item.duration}</span>
      </div>

      <h3 className="text-white text-2xl md:text-3xl font-black mb-1 tracking-tight group-hover:text-[#ff2a2a] transition-colors">
        {item.degree}
      </h3>
      ...
      <div className="inline-block bg-white/10 border border-white/10 rounded-full px-4 py-1.5 text-white text-sm font-bold">
        {item.grade}
      </div>
    </div>
  </div>
...
const Education = () => {
  return (
    <section id="education" className="bg-[#0a0a0a] pt-24 pb-32 px-6 md:px-12 w-full relative overflow-hidden font-sans bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:80px_80px]">
      ...
      {/* Background glow */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[400px] h-[400px] bg-red-600/4 rounded-full blur-[140px] pointer-events-none" />
    </section>
  );
};
