  ...
  return (
    <div
      ...
      className={`relative rounded-2xl p-[1px] group transition-all duration-500 h-full ${
        project.isFlagship
          ? 'md:col-span-2 bg-gradient-to-br from-indigo-500/50 via-white/10 to-indigo-500/30 hover:from-indigo-500 hover:via-indigo-400/30 hover:to-indigo-500/60'
          : 'bg-white/10 light:bg-black/10 hover:bg-white/20 light:hover:bg-black/20'
      }`}
    >
      <div className={`rounded-2xl overflow-hidden h-full flex flex-col backdrop-blur-md transition-all duration-500 ${
        project.isFlagship
          ? 'bg-[#0f0f0f]/95 light:bg-white group-hover:bg-[#0f0f0f]/90 light:group-hover:bg-white md:flex-row'
          : 'bg-[#111111]/90 light:bg-white group-hover:bg-[#111111]/80 light:group-hover:bg-gray-50'
      }`}>
        <div className={project.isFlagship ? 'md:w-[42%] md:shrink-0' : ''}>
          <ProjectThumbnail iconKey={project.icon} isFlagship={project.isFlagship} projectId={project.id} />
        </div>
        ...
        <div className="p-6 md:p-7 flex flex-col flex-1">
          ...
          <div className="flex items-center gap-3 mb-4">
            <span className="w-10 h-10 shrink-0 rounded-xl bg-indigo-500/10 flex items-center justify-center">
              {renderIcon(project.icon, { size: 18, className: 'text-indigo-400', strokeWidth: 1.75 })}
            </span>
            {project.badge && (
              <span className="text-[11px] font-bold tracking-widest uppercase text-indigo-400 bg-indigo-500/10 px-3 py-1 rounded-full border border-indigo-500/20">
                {project.badge}
              </span>
            )}
          </div>
          ...
          <div className="flex items-baseline gap-3 mb-3">
            <span className="text-3xl font-black text-white/10 light:text-black/10 font-serif italic">{project.number}</span>
            <h3 className="text-xl md:text-2xl font-black text-white light:text-gray-900 tracking-tight">{project.title}</h3>
          </div>

          {/* Description */}
          <p className="text-white/60 light:text-gray-500 text-sm leading-relaxed mb-4 line-clamp-3 font-medium">
            {project.description}
          </p>
          ...
          <div className="flex flex-wrap gap-2 mb-6 mt-auto pt-2">
            {project.techTags.slice(0, 6).map((tag) => (
              <span
                key={tag}
                className="px-2.5 py-1 text-[11px] font-bold text-white/70 light:text-gray-600 bg-white/5 light:bg-black/5 rounded-full border border-white/10 light:border-black/10 hover:bg-indigo-500/20 hover:border-indigo-500/30 hover:text-indigo-300 transition-all duration-300 cursor-default"
              >
                {tag}
              </span>
            ))}
